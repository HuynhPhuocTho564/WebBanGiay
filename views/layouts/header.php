...
<header class="bg-white shadow-sm sticky top-0 z-40">
    ...
    <div class="container mx-auto px-4 py-4">
        <div class="flex items-center gap-6">
            ...
            <nav class="hidden lg:flex flex-1 ml-4">
                <ul class="flex items-center gap-10 text-lg font-medium">
                    <li><a href="<?= BASE_URL ?>" class="hover:text-accent transition py-2 <?= empty($currentUrl) ? 'text-accent font-bold' : '' ?>">TRANG CHỦ</a></li>
                    <li><a href="<?= BASE_URL ?>/home/products?gender=Male" class="hover:text-accent transition py-2 <?= strpos($currentUrl, 'products') !== false && ($_GET['gender'] ?? '') === 'Male' ? 'text-accent font-bold' : '' ?>">NAM</a></li>
                    <li><a href="<?= BASE_URL ?>/home/products?gender=Female" class="hover:text-accent transition py-2 <?= strpos($currentUrl, 'products') !== false && ($_GET['gender'] ?? '') === 'Female' ? 'text-accent font-bold' : '' ?>">NỮ</a></li>
                    <li><a href="<?= BASE_URL ?>/home/products?sort=sale" class="hover:text-accent transition py-2 <?= strpos($currentUrl, 'products') !== false && ($_GET['sort'] ?? '') === 'sale' ? 'text-accent font-bold' : '' ?>">SALE</a></li>
                    <li class="relative group">
                        <a href="<?= BASE_URL ?>/home/products" class="hover:text-accent transition flex items-center gap-1 py-2">
                            THƯƠNG HIỆU <i class="fas fa-chevron-down text-xs ml-1"></i>
                        </a>
                        <div class="absolute left-0 top-full mt-1 w-48 bg-white rounded-lg shadow-lg border opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                            <?php 
                            $brands = $brands ?? [];
                            foreach ($brands as $brand): ?>
                            <a href="<?= BASE_URL ?>/home/products?brand=<?= $brand['id'] ?>" 
                               class="block px-4 py-3 hover:bg-gray-50 text-base"><?= $brand['name'] ?></a>
                            <?php endforeach; ?>
                        </div>
                    </li>
                </ul>
            ...
            <form action="<?= BASE_URL ?>/home/products" method="GET" class="hidden lg:block relative w-64" id="searchForm">
                <input type="text" name="q" placeholder="Tìm kiếm sản phẩm..." 
                       id="searchInput"
                       autocomplete="off"
                       class="w-full pl-10 pr-4 py-2.5 bg-gray-100 rounded-full text-base focus:outline-none focus:bg-white focus:ring-2 focus:ring-accent/20"
            ...
            <div class="flex items-center gap-5">
                ...
                <!-- Wishlist - Chỉ hiện cho khách hàng -->
                <?php if (Session::isLoggedIn() && !Session::canAccessAdmin()): ?>
                <a href="<?= BASE_URL ?>/profile/wishlist" class="hidden sm:block text-2xl hover:text-accent" aria-label="Danh sách yêu thích">
                    <i class="far fa-heart" aria-hidden="true"></i>
                </a>
                <?php endif; ?>

                <!-- Cart - Chỉ hiện cho khách hàng -->
                <?php if (!Session::canAccessAdmin()): 
                    // Lấy số lượng giỏ hàng từ database
                    $cartCount = 0;
                    if (Session::isLoggedIn()) {
                        try {
                            $cartCount = Database::getInstance()->count("SELECT COUNT(*) FROM cart WHERE user_id = ?", [Session::userId()]);
                        } catch (Exception $e) {
                            $cartCount = 0; // Bảng cart chưa tồn tại
                        }
                    }
                ?>
                <a href="<?= BASE_URL ?>/cart" class="text-2xl hover:text-accent relative" aria-label="Giỏ hàng">
                ...
                <div class="relative group">
                    <button class="flex items-center gap-2 hover:text-accent">
                        ...
                        <span class="hidden lg:inline text-base"><?= Session::user()['fullname'] ?></span>
                    </button>
                    <!-- Dropdown -->
                    <div class="absolute right-0 top-full mt-2 w-52 bg-white rounded-lg shadow-lg border opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                        <?php if (Session::canAccessAdmin()): ?>
                        <a href="<?= BASE_URL ?>/admin" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 text-base">
                            <i class="fas fa-tachometer-alt w-5"></i> Quản trị
                        </a>
                        <?php endif; ?>
                        <a href="<?= BASE_URL ?>/profile" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 text-base">
                            <i class="fas fa-user w-5"></i> Tài khoản
                        </a>
                        <?php if (!Session::canAccessAdmin()): ?>
                        <a href="<?= BASE_URL ?>/profile/orders" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 text-base">
                            <i class="fas fa-box w-5"></i> Đơn hàng
                        </a>
                        <a href="<?= BASE_URL ?>/profile/wishlist" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 text-base">
                            <i class="fas fa-heart w-5"></i> Yêu thích
                        </a>
                        <?php endif; ?>
                        <hr class="my-1">
                        <a href="<?= BASE_URL ?>/auth/logout" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 text-base text-red-500">
                ...
                <a href="<?= BASE_URL ?>/auth/login" class="flex items-center gap-2 hover:text-accent">
                    ...
                    <span class="hidden lg:inline text-base">Đăng nhập</span>
                </a>
                <?php endif; ?>
            </div>
        </div>
